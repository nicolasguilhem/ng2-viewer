...
export class FormatReader {
	...
	pdfReader = function(data, options, callback, ctx) {
		...
		function renderPage(parent, pageNum, pageObj) {
			...
			if ((pageObj != null)/* && !parent.rendering*/){
				...
				let viewport = pageObj.getViewport( parent.options.zoom.value, 0);
				...
				return pageObj.render({canvasContext : context, viewport : viewport, intent : 'display'}).then( function() {
					...
					if (options.controls.filmStrip) {
						let item = context.getImageData( 0, 0, viewport.width, viewport.height);
		...
		this.refresh = function() {
				...
				if (options.controls.filmStrip) {
					...
					that.data = [];
					for (let p = 1; p <= options.controls.totalPage; p++) {
					...
					if (that.currentPage != that.options.controls.numPage) {
						...
						renderPage(that, that.options.controls.numPage, that.currentPage);
					}
				}
		};

		this.reader.onload = function() {
			let data = new Uint8Array(that.reader.result);
	...
	tiffReader = function(data, options, callback) {
		...
		this.refresh = function() {
			...
			if (that.options.controls.filmStrip) {
				that.images = [];
				for(let p=0; p < that.tiff.countDirectory(); p++) {
		...
		try {
			...
			this.reader.onload = function() {
				...
				adctx.decodeAudioData( that.reader.result , function(buffer) {
					let gradient = ctx.createLinearGradient(0,0,0,that.height);
					gradient.addColorStop(1,'#000000');
					gradient.addColorStop(0.75,'#ff0000');
					gradient.addColorStop(0.25,'#ffff00');
					gradient.addColorStop(0,'#ffffff');
					// setup a javascript node
					let javascriptNode = adctx.createScriptProcessor(2048, 1, 1);
					let source = adctx.createBufferSource();
					options.adsrc = source;
					// tell the source which sound to play
					source.buffer = buffer;
					// setup a analyzer
					let analyser = adctx.createAnalyser();
